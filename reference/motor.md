...
A [RotationalMotor](#rotationalmotor) can power a [HingeJoint](#hingejoint)
(resp. a [Hinge2Joint](#hinge2joint)) when set inside the `device` (resp.
`device` or `device2`) field of these nodes. It produces then a rotational
motion around the choosen axis. Likewise, a [LinearMotor](#linearmotor) can
power a [SliderJoint](#hingejoint), producing a sliding motion along its axis.
...
[JointParameters](#jointparameters). Rotational motors units are expressed in
...
the `position` field in [JointParameters](#jointparameters)). Soft limits
...
limits (`minStop` and `maxStop`, see [JointParameters](#jointparameters)) are
...
{[C++](#cpp_motor)}, {[Java](#java_motor)}, {[Python](#python_motor)}, {[Matlab](#matlab_motor)}
...
Therefore, the `physics` and `boundingObject` fields of related [Solid](#solid)
nodes must be defined for these functions to work properly. Moreover they don't
work for [Motor](#motor) nodes used to power a [Track](#track) node.
