...
If the [TouchSensor](#touchsensor) node doesn't have any dynamic ancestor [Solid](#solid) node, i.e. any node whose `physics` field is set, it will be attached to the static environment with an ODE fixed joint.
