...
vehicle.
- `radarCrossSection`: Defines the `radarCrossSection` of the vehicle, if greater than 0, this vehicle is a potential target for any [Radar](../reference/radar.md) device. It measures the vehicle ability to reflect radar signals in the direction of the radar receiver.
- `recognitionColors`: Defines the `recognitionColors` of the vehicle. If not empty, this vehicle may be recognized by any [Camera](../reference/camera.md) device with recognition capability (i.e. with a [Recognition](../reference/recognition.md) recognition node).
