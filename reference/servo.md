...
the [HingeJoint](#hingejoint), [RotationalMotor](#rotationalmotor) and
[LinearMotor](#linearmotor) nodes. Therefore, you should use these nodes instead
of the [Servo](#servo) node.
...
the [Transform](#transform) node. These two fields represent the initial
...
by the `basicTimeStep` field of the [WorldInfo](#worldinfo) node (converted in
...
*m<sub>1</sub>*. *m<sub>0</sub>* is defined by the [Physics](#physics) node in
the parent of the [Servo](#servo). The mass *m<sub>1</sub>* is defined by the
[Physics](#physics) node of the [Servo](#servo). The value *x<sub>0</sub>*
...
Note that it is necessary to specify the [Physics](#physics) and the
...
`boundingObject` can be specified as a [Sphere](#sphere) with a radius of 1
millimeter. A [Sphere](#sphere) is the preferred choice because this is the
cheapest shape for the collision detection. And the `physics` field can use a
[Physics](#physics) node with default values.
...
Please note the dummy [Physics](#physics) and the 1 millimeter [Sphere](#sphere)
as dummy `boundingObject`.
...
{[C++](#cpp_servo)}, {[Java](#java_servo)}, {[Python](#python_servo)}, {[Matlab](#matlab_servo)}
