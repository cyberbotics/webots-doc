...
invariants out of the loop). The `wb_supervisor_node_get_from_def` function
...
In the `while` loop, the `wb_supervisor_field_get_sf_vec3f` function call is used to
...
the `wb_motor_set_postion` function for each motor involved in the locomotion. After each
...
`trans_field` is used both to get (using the `wb_supervisor_field_get_sf_vec3f` function)
and to set (using the `wb_supervisor_field_set_sf_vec3f` function) the field's value.
...
the `wb_robot_step` function call is placed in the innermost loop. This allows the
motor positions to be updated at each iteration of the loop. If
the `wb_robot_step` function call was placed anywhere else, this would not work.
