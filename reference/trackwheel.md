...
Note that in order to automatically generate the track belt the geometries animation has to be enabled, i.e. the `animatedGeometry` field of the parent [Track](track.md) node has to be defined as well and the `geometriesCount` field have to be greater than 0.
