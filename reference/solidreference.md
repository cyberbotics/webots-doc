...
A [SolidReference](solidreference.md#solidreference) can be used inside the
`endPoint` field of a [Joint](joint.md#joint) node to refer either to an
existing [Solid](solid.md#solid) or to the static environment. Mechanical loops
can be closed this way. The only constraint when referring to a
[Solid](solid.md#solid) is that both [Solid](solid.md#solid) and
[Joint](joint.md#joint) must be descendants of a common upper
