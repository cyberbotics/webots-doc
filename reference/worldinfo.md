...
This seed has for example an influence on the noise pattern generated by some sensors.
The value of the seed should be either non-negative or -1, if the value is -1 a time based seed is then used.
Note that using a time based seed implies by definition non-reproducible simulations.
