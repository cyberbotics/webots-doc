...
Hence it is possible to call the `Robot`'s methods, such as, e.g., the `step` or
`getLED` functions, from the `DifferentialWheels` and `Supervisor` controllers. But it
...
the `simulationRevert` function from a `DifferentialWheels` controller.
...
functions like the `wb_robot_init` and `wb_robot_cleanup` functions. The necessary
...
`Robot` class, like the `getDistanceSensor` or the `getTouchSensor` functions. There is no
