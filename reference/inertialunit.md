...
the [WorldInfo](#worldinfo) node. If you would like to measure an acceleration
or an angular velocity, please use the [Accelerometer](#accelerometer) or
[Gyro](#gyro) node instead. The [InertialUnit](#inertialunit) node must be
placed on the [Robot](#robot) so that its *x*-axis points in the direction of
the [Robot](#robot)'s forward motion (longitudinal axis). The positive *z*-axis
must point towards the [Robot](#robot)'s right side, e.g., right arm, right wing
(lateral axis). The positive *y*-axis must point to the [Robot](#robot)'s up/top
direction. If the [InertialUnit](#inertialunit) has this orientation, then the
*roll*, *pitch* and *yaw* angles correspond to the usual automotive, aeronautics
or spatial meaning.  More precisely, the [InertialUnit](#inertialunit) measures
the Tait-Bryan angles along *x*-axis (roll), *z*-axis (pitch) and *y*-axis
(yaw). This convention is commonly referred to as the *x-z-y* extrinsic
sequence; it corresponds to the composition of elemental rotations denoted by
YZX. The reference frame is made of the unit vector giving the north direction,
the opposite of the normalized gravity vector and their cross-product (see
[WorldInfo](#worldinfo) to customize this frame).
...
{[C++](#cpp_inertial_unit)}, {[Java](#java_inertial_unit)}, {[Python](#python_inertial_unit)}, {[Matlab](#matlab_inertial_unit)}
...
opposite direction of the gravity ([WorldInfo](#worldinfo) defines the `gravity`
vector).
...
placed on the [Robot](#robot) with a standard orientation, then the *pitch*
angle is negative when the [Robot](#robot) is going down, and positive when the
robot is going up.

The *yaw* angle indicates the unit orientation, in the interval [-π,π], with
respect to [WorldInfo](#worldinfo).`northDirection`. The *yaw* angle is zero
