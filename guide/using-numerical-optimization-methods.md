...
positions should be reset using the `wb_motor_set_position` function and the robot
...
controller that called the `wb_supervisor_simulation_revert` function, this is usually the
...
the `wb_supervisor_simulation_revert` function and reloaded when the `Supervisor`
...
If this technique is used with Genetic Algorithms for example, then the
`optimizer_save_state` function should save at least all the genotypes and fitness
...
termination, e.g. like the C standard `system` function does. On the contrary, the
