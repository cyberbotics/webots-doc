...
must be defined for these functions to work properly. Moreover they don't work
for `Motor` nodes used to power a `Track` node.
...
the `physics` and `boundingObject` fields of the `Motor` node must be defined
