...
- **Extend**: Displayed only when a PROTO node is selected, replaces the selected PROTO node with an equivalent basic node (non-PROTO node) containing only basic nodes.
