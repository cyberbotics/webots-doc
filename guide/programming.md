...
to the robot, then use the `wb_robot_get_device`, `wb_gps_enable` and
...
In the physics plugin you can use the ODE's `dBodyGetLinearVel` and
...
For that reason the control step specified with each `wb_robot_step` function call must be a
...
control step argument passed to the `wb_robot_step` function can be 16, 32, 48, 64, 80,
128, 1024, etc.
