...
(legged, humanoid, vehicle, etc.) needs to use [Robot](#robot) as base node.
...
must be [Solid](#solid) nodes, and they must be named "right wheel" and "left
...
contains [Physics](#physics) nodes in its body and wheels. In this mode, the
...
[Physics](#physics) nodes. The "physics" mode is the most realistic but also the
slowest simulation mode.

#### Kinematics mode

When a [DifferentialWheels](#differentialwheels) does not have
[Physics](#physics) nodes then it is simulated in "kinematics" mode. In the
...
{[C++](#cpp_differential_wheels)}, {[Java](#java_differential_wheels)}, {[Python](#python_differential_wheels)}, {[Matlab](#matlab_differential_wheels)}
...
that in a kinematic simulation (with no [Physics](#physics) node set) when a
...
appropriate [Physics](#physics) nodes), the wheels may slip depending on their
